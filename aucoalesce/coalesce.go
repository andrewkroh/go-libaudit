...
func addSockaddrRecord(sockaddr *auparse.AuditMessage, event *Event) {
	...
	if err != nil {
		event.Warnings = append(event.Warnings,
			fmt.Errorf("failed to parse SOCKADDR message: %w", err))
		return
	}
...
func addPathRecord(path *auparse.AuditMessage, event *Event) {
	...
	if err != nil {
		event.Warnings = append(event.Warnings,
			fmt.Errorf("failed to parse PATH message: %w", err))
		return
	}
...
func addExecveRecord(execve *auparse.AuditMessage, event *Event) {
	...
	if err != nil {
		event.Warnings = append(event.Warnings,
			fmt.Errorf("failed to parse EXECVE message: %w", err))
		return
	}
	...
	if err != nil {
		event.Warnings = append(event.Warnings,
			fmt.Errorf("failed to convert argc='%v' to number: %w", argc, err))
		return
	}
	...
	for i := 0; i < int(count); i++ {
		...
		if !found {
			event.Warnings = append(event.Warnings,
				fmt.Errorf("failed to find arg %v", key))
			return
		}
...
func addFieldsToEventData(msg *auparse.AuditMessage, event *Event) {
	...
	for k, v := range data {
		if _, found := event.Data[k]; found {
			event.Warnings = append(event.Warnings,
				fmt.Errorf("duplicate key (%v) from %v message", k, msg.RecordType))
			continue
		}
		event.Data[k] = v
	}
...
func applyNormalization(event *Event) {
	...
	if len(norm.Subject.PrimaryFieldName.Values) > 0 {
		...
		if err != nil {
			event.Warnings = append(event.Warnings,
				fmt.Errorf("failed to set subject primary using keys=%v because "+
					"they were not found", norm.Subject.PrimaryFieldName.Values))
		}
	}
	...
	if len(norm.Subject.SecondaryFieldName.Values) > 0 {
		...
		if err != nil {
			event.Warnings = append(event.Warnings, fmt.Errorf("failed to set "+
				"subject secondary using keys=%v because they were not found",
				norm.Subject.SecondaryFieldName.Values))
		}
	}
	...
	if len(norm.Object.PrimaryFieldName.Values) > 0 {
		...
		if err != nil {
			event.Warnings = append(event.Warnings, fmt.Errorf("failed to set "+
				"object primary using keys=%v because they were not found",
				norm.Object.PrimaryFieldName.Values))
		}
	}
	...
	if len(norm.Object.SecondaryFieldName.Values) > 0 {
		...
		if err != nil {
			event.Warnings = append(event.Warnings, fmt.Errorf("failed to set "+
				"object secondary using keys=%v because they were not found",
				norm.Object.SecondaryFieldName.Values))
		}
	}
	...
	if len(norm.How.Values) > 0 {
		...
		if err != nil {
			event.Warnings = append(event.Warnings, fmt.Errorf("failed to set "+
				"how using keys=%v because they were not found", norm.How.Values))
		}
	}
